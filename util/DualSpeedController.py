...
class DualSpeedController(SpeedController):
    ...
    def IsWiringSUCK(self):
        #If only one of the motors is drawing current, the other isn't wired right
        if abs(self.controller1.GetOutputCurrent() - self.controller2.GetOutputCurrent()) > 20:
            return True
        else:
            return False
